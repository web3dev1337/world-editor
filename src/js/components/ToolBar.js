...
const ToolBar = ({
  ...
  const handleGenerateBlocks = (fillInterior = true) => {
    ...
    // Generate blocks based on current position and dimensions
    const blocks = [];
    const startPos = environmentBuilder.getCurrentPosition();
    ...
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        for (let z = 0; z < length; z++) {
          ...
          // Skip interior blocks if fillInterior is false
          if (!fillInterior && isInterior) continue;
    ...
    environmentBuilder.addBlocks(blocks);
  ...
  const handleGenerateBorder = () => {
    ...
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        for (let z = 0; z < length; z++) {
          // Only add blocks that form the border
          if (
            x === 0 ||
            x === width - 1 ||
            y === 0 ||
  ...
  const generateTerrain = () => {
    ...
    for (let x = 0; x < width; x++) {
      for (let z = 0; z < length; z++) {
        const terrainHeight = Math.floor(noiseMap[x][z]);
        ...
        <div className="control-group">
          <div className="control-button-wrapper">
            ...
            <Tooltip text="Import complete asset pack (includes map and textures)">
              <button
                onClick={() =>
                  document.getElementById("assetPackInput").click()
                }
                className="control-button import-export-button"
              >
                Asset Pack
